...
- **投稿の同期 (`posts sync`):** まずサーバーから最新の投稿情報を取得してローカルキャッシュを更新します。その後、サーバーの更新と競合しないローカルの変更点をサーバーにプッシュ（送信）します。実行後には、どの投稿が更新・プッシュされたかの詳細なレポートが表示されます。このコマンドは投稿と同時に**分類（カテゴリ・タグ）**のファイルもローカルに生成・更新します。
